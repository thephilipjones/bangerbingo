...
import { determineInitialPage, applyPlayerEvent, copyRoomCode, connectAsHost } from '../lib/ws.ts'
...
// ── connectAsHost — WS connection (AC 8) ─────────────────────────────────
...
describe('connectAsHost', () => {
  ...
  beforeEach(() => {
    mockInstance = { onmessage: null, onerror: null, onclose: null }
  ...
  it('connects to /ws?code=<code> without a name param', () => {
    connectAsHost('ABCD', { onConnect: vi.fn(), onPlayerJoined: vi.fn(), onPlayerLeft: vi.fn(), onAuthDegraded: vi.fn(), onDisconnected: vi.fn() })
    expect(MockWebSocket).toHaveBeenCalledWith('ws://localhost:3000/ws?code=ABCD')
  })
  ...
  it('ignores unparseable messages without throwing', () => {
    connectAsHost('ABCD', { onConnect: vi.fn(), onPlayerJoined: vi.fn(), onPlayerLeft: vi.fn(), onAuthDegraded: vi.fn(), onDisconnected: vi.fn() })
    expect(() => mockInstance.onmessage!({ data: 'not json' })).not.toThrow()
  })

  it('returns the WebSocket instance', () => {
    const ws = connectAsHost('ABCD', { onConnect: vi.fn(), onPlayerJoined: vi.fn(), onPlayerLeft: vi.fn(), onAuthDegraded: vi.fn(), onDisconnected: vi.fn() })
    expect(ws).toBe(mockInstance)
  })

  it('calls onDisconnected on onerror', () => {
    const onDisconnected = vi.fn()
    connectAsHost('ABCD', { onConnect: vi.fn(), onPlayerJoined: vi.fn(), onPlayerLeft: vi.fn(), onAuthDegraded: vi.fn(), onDisconnected })
    mockInstance.onerror!()
    expect(onDisconnected).toHaveBeenCalled()
  })

  it('calls onDisconnected on abnormal close', () => {
    const onDisconnected = vi.fn()
    connectAsHost('ABCD', { onConnect: vi.fn(), onPlayerJoined: vi.fn(), onPlayerLeft: vi.fn(), onAuthDegraded: vi.fn(), onDisconnected })
  ...
  it('does not call onDisconnected on clean close (code 1000)', () => {
    const onDisconnected = vi.fn()
    connectAsHost('ABCD', { onConnect: vi.fn(), onPlayerJoined: vi.fn(), onPlayerLeft: vi.fn(), onAuthDegraded: vi.fn(), onDisconnected })
